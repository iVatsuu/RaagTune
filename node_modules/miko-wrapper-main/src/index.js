...
class MikoWrapper {
  /**
   * @description The main constructor / class for miko-wrapper
   * @param {Client} client Your discord bot client
   * @param {Array<MikoNode>} nodes Your lavalink nodes
   * @param {MikoMessages} messages The messages for your bot
   */
  ...
  async searchAndPlay(node, query, service, message) {
    ...
    if (indexTrack.info.isStream && isRadio === false) return false;
  ...
  /**
   * @description Will return true if it is a valid url, false if it isn't. This is a utility,
  ...
  getQueue(message) {
    return this.queue.has(message.guild.id)
      ? {
          ...
          skipVotes: this.queue.get(message.guild.id).skipVotes,
        }
      : false;
  }
  /**
  ...
  async resume(message) {
   ...
   /**
   * @param {Message} message The message instance
   * @param {Boolean} loop Loop True or false
   ...
   */
  async loopOne(message) {
    if (!this.getQueue(message)) return false;
    this.queue.get(message.guild.id).loop = false;
    this.queue.get(message.guild.id).repeat = true;
    console.log(`Looping[One] For : ${message.guild.name}`);
     return true;
    }
   /**
   * @param {Message} message The message instance
  ...
  async loopAll(message) {
    if (!this.getQueue(message)) return false;
    this.queue.get(message.guild.id).repeat = false;
    this.queue.get(message.guild.id).loop = true;
    console.log(`Looping[All] For : ${message.guild.name}`);
     return true;
    }
       /**
   * @param {Message} message The message instance
   ...
   * @description Will return true if success, false if failed. Will make the bot leave the voice channel and delete the queue for that guild.
   */
  async loopOff(message) {
    if (!this.getQueue(message)) return false;
    this.queue.get(message.guild.id).repeat = false;
    this.queue.get(message.guild.id).loop = false;
    console.log(`Looping[Off] For : ${message.guild.name}`);
     return true;
    }
/**
 * 

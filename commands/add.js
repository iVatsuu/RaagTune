...
module.exports = {
    ...
    run: async(client, message , args) => {
        const msg = await message.channel.send(`📡 Adding Song to the Playlist! This might take a few seconds...`);
        ...
        async function addSongs(isPlaylist){
            await mongo().then(async (mongoose) =>{
                try {
                    await playlistSchema.findOne({
                        ...
                        if(!p){
                            ...
                            const embed = new MessageEmbed()
						        ...
						        .setDescription(`✅ Added song to: **${p.name}**.\n${playlistMessage}`)
                    ...
                    msg.edit(`❌ Failed to Add song to playlist: ${error}`)
                    console.log(error);
                }finally{
